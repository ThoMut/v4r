...
void MainWindow::on_StartModelling_clicked()
{
    ...
    //m_glviewer->drawBoundingBox(false);
    m_ui->statusLabel->setText("Status: Modelling started | Turn the Object 360deg");
}
...
void MainWindow::finishedCreateMesh()
{
  ...
  QString model_name = QInputDialog::getText(this, tr("Store model"), tr("Model name:"), QLineEdit::Normal, text, &ok_save);
  ...
  if ( ok_save && model_name.isNull() == false )
  {
      ...
      if (ok_save)
      {
          ...
          m_ui->statusLabel->setText("Status: Save model...");
